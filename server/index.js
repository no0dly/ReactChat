...
socketServer.events.on('connection', socket => {
    setTimeout(() => {
        if (!manager.getUser('socket', socket)) {
            console.log(new Date(), 'Register timeout was expired');
...
socketServer.events.on('close', socket => {
    ...
    if (user) {
        console.log(new Date(), `User ${user.login} was disconnected`);
...
socketServer.events.on('message', (socket, message) => {
    switch (message.op) {
        case 'reg':
        {
            ...
            console.log(new Date(), `User ${message.data.login} was registered`);
            socketServer.send({
                op: 'token',
                sourceOp: message.op,
                token: token,
        ...
        case 'message':
        {
            ...
            console.log(new Date(), `User ${fromUser.login} sending message...`);
            messages.push({
...
httpServer.events.on('upload', (fields, files) => {
    ...
    console.log(new Date(), `Photo of ${user.login} was loaded`);
